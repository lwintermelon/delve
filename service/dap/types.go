...
//	-- "ttd-gdbserver" - replays a Windows Time Travel Debugging (.run)
//	       trace through the external ttd-gdbserver stub.
//
...
func isValidLaunchMode(mode string) bool {
	switch mode {
	case "exec", "debug", "test", "replay", "core", "ttd-gdbserver":
		return true
	}
	return false
}
...
type LaunchConfig struct {
	...
	//   "ttd-gdbserver": replays a Windows Time Travel Debugging .run trace
	//     through the external ttd-gdbserver stub.
	...
	// TtdTracePath is the .run trace file path for ttd mode.
	// Relative path is interpreted as a path relative to Delve's
	// current working directory.
	// This is required for "ttd" mode but unused in other modes.
	TtdTracePath string `json:"ttdTracePath,omitempty"`
