...
func New(docCall bool) *cobra.Command {
	...
	must(rootCommand.RegisterFlagCompletionFunc("backend", cobra.FixedCompletions([]string{"default", "native", "lldb", "rr", "ttd-gdbserver"}, cobra.ShellCompDirectiveNoFileComp)))
...
https://github.com/mozilla/rr
	...
	ttdCommand := &cobra.Command{
		Use:     "ttd-gdbserver <tracefile> [executable]",
		Aliases: []string{"ttd"},
		Short:   "Replays a Windows Time Travel Debugging (.run) trace via ttd-gdbserver.",
		Long: `Replays a Windows Time Travel Debugging trace.

The ttd command opens a .run trace generated by the Windows Time Travel
Debugging recorder (ttd.exe) via the 'ttd-gdbserver' GDB stub.
ttd-gdbserver must be installed and reachable through PATH, or the
DELVE_TTD_SERVER environment variable must point at the executable.`,
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return errors.New("you must provide a path to a .run trace file")
			}
			return nil
		},
		Run: func(cmd *cobra.Command, args []string) {
			backend = "ttd-gdbserver"
			exe := ""
			if len(args) > 1 {
				exe = args[1]
			}
			os.Exit(execute(0, []string{exe}, conf, args[0], debugger.ExecutingOther, args, buildFlags))
		},
		ValidArgsFunction: func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			if len(args) > 1 {
				return nil, cobra.ShellCompDirectiveNoFileComp
	...
	rootCommand.AddCommand(ttdCommand)
